...
def _crop_display_name(crop_name):
...
def _nutrient_status_lines(actual_n, actual_p, actual_k, reference_n, reference_p, reference_k):
    status_lines = []
    values = [
    ...
    for name, actual, reference in values:
        ...
        if abs(difference) <= 5:
            status_lines.append(
                "{0}: soil {1}, crop target {2} (close to target).".format(name, actual, reference)
            )
        elif difference > 0:
            status_lines.append(
                "{0}: soil {1}, crop target {2} (higher by {3}).".format(name, actual, reference, difference)
            )
        else:
            status_lines.append(
                "{0}: soil {1}, crop target {2} (lower by {3}).".format(name, actual, reference, abs(difference))
            )

    return status_lines


def _major_issue_text(crop_display_name, recommendation_key):
...
def build_fertilizer_plan(crop_name, reference_n, reference_p, reference_k, actual_n, actual_p, actual_k):
    crop_family = _crop_family(crop_name)
    crop_display_name = _crop_display_name(crop_name)
    ...
    if recommendation_key == BALANCED_FERTILIZER_KEY:
        basal_lines.append("Apply neem cake 25 kg/acre as a mild basal maintenance input.")
        vegetative_lines.append("Use 19:19:19 water-soluble fertilizer up to 5 kg/acre only if crop growth slows during the vegetative stage.")
        flowering_lines.append("Use SOP or equivalent potash support up to 5 kg/acre only if flowering or fruit set looks weak.")
        quantity_lines.extend([
            "Neem cake: 25 kg/acre",
            "19:19:19 maintenance feed: up to 5 kg/acre",
            "Flowering support feed: up to 5 kg/acre",
        ])
        organic_lines.extend([
            "Continue only mature compost, mulching, and light biofertilizer use if the crop is growing normally.",
            "Jeevamrut or vermicompost tea can be used as a light maintenance input instead of a strong chemical correction.",
        ])
    else:
        ...
        if basal_urea > 0:
            basal_lines.append("Urea: {0} kg/acre at basal stage.".format(basal_urea))
        elif actual_n > reference_n:
            basal_lines.append("Do not add urea at basal stage because nitrogen is already above target.")

        if basal_p > 0:
            basal_lines.append("{0}: {1} kg/acre at basal stage.".format(p_source, basal_p))
        elif actual_p > reference_p:
            basal_lines.append("Skip phosphorus-heavy basal fertilizer for now because phosphorus is already high.")

        if basal_mop > 0:
            basal_lines.append("MOP: {0} kg/acre at basal stage.".format(basal_mop))
        elif actual_k > reference_k:
            basal_lines.append("Skip potash-heavy basal fertilizer for now because potassium is already high.")

        if vegetative_urea > 0:
            vegetative_lines.append("Top-dress urea: {0} kg/acre at 20-25 days after sowing or transplanting.".format(vegetative_urea))
        if vegetative_p > 0:
            vegetative_lines.append("Apply remaining {0}: {1} kg/acre during early vegetative growth if root development is weak.".format(p_source, vegetative_p))
        if vegetative_mop > 0:
            vegetative_lines.append("Apply MOP: {0} kg/acre during vegetative growth.".format(vegetative_mop))
        if not vegetative_lines:
            vegetative_lines.append("No major vegetative-stage chemical correction is needed if crop growth remains normal.")

        if flowering_urea > 0:
            flowering_lines.append("Use light urea support: {0} kg/acre only if foliage looks pale before flowering.".format(flowering_urea))
        if flowering_p > 0:
            flowering_lines.append("Apply {0}: {1} kg/acre only if flowering is delayed and phosphorus shortage symptoms remain visible.".format(p_source, flowering_p))
        if flowering_mop > 0:
            flowering_lines.append("Apply MOP: {0} kg/acre around flowering, boll formation, tuber bulking, or fruit set.".format(flowering_mop))
        if not flowering_lines:
            flowering_lines.append("No heavy flowering-stage chemical correction is needed if the crop remains healthy.")

        quantity_lines.extend([
            "Urea total: {0} kg/acre".format(urea_total) if urea_total > 0 else "Urea total: 0 kg/acre for now",
            "{0} total: {1} kg/acre".format(p_source, p_total) if p_total > 0 else "{0} total: 0 kg/acre for now".format(p_source),
            "MOP total: {0} kg/acre".format(mop_total) if mop_total > 0 else "MOP total: 0 kg/acre for now",
        ])

        if n_gap > 0:
            organic_lines.append("For nitrogen support, use 300-500 kg vermicompost or well-rotted manure per acre in split applications.")
        elif actual_n > reference_n:
            organic_lines.append("Avoid fresh manure and heavy urea for now; use only mature compost in light quantity.")

        if p_gap > 0:
            organic_lines.append("For phosphorus support, use rock phosphate or bone meal with compost at basal stage.")
        elif actual_p > reference_p:
            organic_lines.append("Avoid DAP and phosphorus-heavy manure until the crop uses the available phosphorus.")

        if k_gap > 0:
            organic_lines.append("For potassium support, use wood ash or banana-residue compost in a controlled quantity.")
        elif actual_k > reference_k:
            organic_lines.append("Avoid muriate of potash and other potash-heavy inputs for now.")

        if not organic_lines:
            organic_lines.append("Use only light compost and biofertilizer support because no strong organic correction is needed.")

    return {
        "crop_name": crop_display_name,
        "recommendation_key": recommendation_key,
        "summary": _major_issue_text(crop_display_name, recommendation_key),
        "status_lines": _nutrient_status_lines(
            actual_n, actual_p, actual_k, reference_n, reference_p, reference_k
        ),
        ...
        "note": "This is an approximate per-acre schedule derived from the crop reference NPK in the dataset and your submitted soil values. Adjust it with local agronomy guidance, irrigation availability, and crop age.",
    }
